...
def _get_transitions():
    ...
    transitions = list(
        map(lambda t: [knowledge_env.env.maze.perception(t[0]), t[1],
                       knowledge_env.env.maze.perception(t[2])], transitions))
...
def _save_experiment_json(agent, env, explore_metrics, exploit_metrics_1, exploit_metrics_2, exploit_metrics_3, data_path, explore_time, exploit_time):
    ...
    full_path = DATA_PATH + '/ACS2/' + data_path
...
def run_acs2_experiment():
    for i in range(REPEAT_START, REPEAT_START + REPEAT):
        ...
        _run_experiment(agent, os.path.join(f'acs2', f'{i}'))
