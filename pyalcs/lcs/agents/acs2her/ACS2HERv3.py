    ...
    def _run_trial_explore(self, env, time,
        ...
        for index, step in enumerate(trial_steps):
            ...
            self.memory.add(ReplayMemorySample(
                ACS2HERv3.state_goal_concat(state, self.main_goal),
                action,
                reward,
                ACS2HERv3.state_goal_concat(next_state, self.main_goal),
                done))
            ...
            for goal in additional_goals:
                ...
                self.memory.add(ReplayMemorySample(
                    ACS2HERv3.state_goal_concat(state, goal),
                    action,
                    new_reward,
                    ACS2HERv3.state_goal_concat(next_state, goal),
                    False))
