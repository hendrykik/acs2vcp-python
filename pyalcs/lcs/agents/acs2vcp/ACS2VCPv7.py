...
def learning_worker_with_indices(args):
    ...
    """
    try:
        agent_config, population_copy, time_arg, agent_id, steps, experiences = args        
        ...
        for idx, exp in enumerate(experiences):
            ...
            ClassifiersList.apply_alp(
                ...
                exp.state, exp.action, exp.next_state, time_arg + steps,
            ...
            if temp_agent.cfg.do_ga:
                ClassifiersList.apply_ga(
                    time_arg + steps, temp_agent.population,
        ...
        return agent_id, temp_agent.population
...
class ACS2VCPv7(Agent):
    ...
    def _run_trial_explore(self, env, time_arg, current_trial=None) -> TrialMetrics:
        ...
        try:
            ...
            return self._run_trial_explore_sequential(env, time_arg, current_trial)
        
        print('Przejscie:  ', time.time() - start_time)
        ...
        print('Dodanie próbek do bufora:  ', time.time() - start_time)
                
        self.learn_parallel(time_arg, trial_steps_all)
    ...
    def learn_parallel(self, time_arg, trial_steps_all):
        """Parallel learning"""
        try:
            ...
            for trail_num in range(max_steps):
                ...
                for agent_id, trail_steps in trial_steps_all.items(): 
                    if trail_num <= len(trail_steps):
                        ...
                        all_learning_tasks.append((
                            self.cfg,
                            self.ensemble_heads[agent_id].population,
                            time_arg,
                            agent_id,
                            trail_num,
                            experiences
                ...
                if len(all_learning_tasks) > 0:
                    ...
                    print('Uczenie:  ', time.time() - start_time)
                    
                    for agent_id, updated_population in results: #, experiences_n, indices_n in results:
                        self.ensemble_heads[agent_id].population = updated_population
                        
                    start_time = time.time()
                    # self.recalculate_all_priorities()
                    # print('Zmiana wszystkich priorytetów:  ', time.time() - start_time)
                    ...
                    print('Zmiana wybranych priorytetów:  ', time.time() - start_time)
            ...
            for agent_id, trial_steps in trial_steps_all.items():
                for i in range(len(trial_steps)):
                    self.learn(time_arg, i, agent_id)
    ...
    def recalculate_all_priorities(self):
        ...
        if all_sigmas:
            self.memory.sigma2_max = max(all_sigmas)
        
        new_priorities = []
        for sigma2 in all_sigmas:
            p = (self.memory.sigma2_max - sigma2 + self.memory.eps) ** self.memory.T
            new_priorities.append(p)
        
        self.memory.sigmas = all_sigmas
        self.memory.priorities = new_priorities

    def _run_trial_exploit(self, env, time_arg=None, current_trial=None) -> TrialMetrics:
    ...
    def learn(self, time_arg, steps, i):
        experiences, indices, weights = self.memory.sample()
        for idx, exp in enumerate(experiences):
            ...
            if self.ensemble_heads[i].cfg.do_ga:
                ClassifiersList.apply_ga(
                    time_arg + steps, self.ensemble_heads[i].population,
                    ClassifiersList() if exp.done else er_next_match_set, er_action_set,
