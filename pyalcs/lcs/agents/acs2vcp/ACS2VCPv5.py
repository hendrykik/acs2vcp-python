...
def learning_worker_with_indices(args):
    ...
    """
    try:
        agent_config, population_copy, time, agent_id, trail_steps, memory = args        
        temp_agent = ACS2HER(agent_config, population_copy)   
        experiences_n = []
        indices_n = []
        
        for steps in range(len(trail_steps)):
...
class ACS2VCPv5(Agent):
    ...
    def learn_parallel(self, time, trial_steps_all):
        """Corrected parallel learning - maintain exact sequential order"""
        try:
            all_experiences = [] 
            all_indices = []
            ...
            for _, (head, trail_steps) in enumerate(trial_steps_all.items()):
                all_learning_tasks = []
                for agent_id in range(len(self.ensemble_heads)):    
