    ...
    def _run_trial_explore(self, env, time,
        ...
        for i in range(len(self.ensemble_heads)):
            ...
            while not done:
                ...
                state_with_goal = ACS2HER.state_goal_concat(state, self.ensemble_heads[i].main_goal)
                assert len(state_with_goal) == self.ensemble_heads[i].cfg.classifier_length

                match_set = self.ensemble_heads[i].population.form_match_set(state_with_goal)
