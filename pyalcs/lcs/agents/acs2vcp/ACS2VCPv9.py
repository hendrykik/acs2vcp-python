...
from lcs.agents.acs2vcp.PrioritizedReplayBufferv2 import PrioritizedReplayBufferv2
...
class ACS2VCPv9(Agent):
    """
    Optimized ACS2 with VCP (Variance-based Curiosity Prioritization).
    
    Improvements over v8:
    - Uses PrioritizedReplayBufferv2 with cached probabilities
    - Reduced priority update frequency (configurable via priority_update_freq)
    """
    ...
    def __init__(self,
                 ...
                 buffer_T: float = 1.0,
                 priority_update_freq: int = 5) -> None:
        base_population = population or ClassifiersList()
        
        self.memory_raw = []
        self.memory = PrioritizedReplayBufferv2(
        ...
        # How often to update priorities (every N steps)
    ...
    def learn_new_buffer(self, time_arg, trial_steps_all):
        ...
        for trail_num in range(max_steps):
            ...
            if len(all_learning_tasks) > 0:
                ...
                # Only update priorities every N steps to reduce Q-computation overhead
                if trail_num % self.priority_update_freq == 0:
                    new_sigmas = []
                    new_prior = []
                    final_indices = []
